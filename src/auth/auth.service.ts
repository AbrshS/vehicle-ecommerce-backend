...
export class AuthService {
  ...
  async signin(@Body() dto: AuthDto): Promise<Object> {
    ...
    if (user.status === 'BLOCKED') {
      throw new NotAcceptableException('blocked user');
    }
    ...
    if (!passwordMatch) {
      throw new UnauthorizedException('password mis-mach');
    }
  ...
  async getResetCode(userId: string, email: string): Promise<string> {
    const resetCode = randomPasswordGenerator.generateRandomDecimalPassword();

    // fetch secret code from .env file.
    const secret = this.config.get('JWT_SECRET');

    // a payload for generated token.
    const privateInfo = {
      sub: userId,
      resetCode: resetCode,
      email,
    ...
    const token = await this.jwtService.signAsync(privateInfo, {
      expiresIn: '5m', // valid only for 5 minute.
      secret: secret,
    });
  ...
  async isValidToken(token: string): Promise<Object> {
    try {
      ...
      throw new RequestTimeoutException('expired token');
    } catch (error) {
      throw new RequestTimeoutException('expired token');
    }
  }
  ...
  async resetPassword(dtoRP: ResetPasswordDto): Promise<Object> {
    ...
    if (isValidToken) {
      ...
      return {
        message: 'password have been successfully reset.',
      };
    } else {
      return new RequestTimeoutException('expired code!');
    }
  }
  ...
  async changePassword(dtoCP: ChangePassword): Promise<Object> {
    ...
    if (isMuch) {
      ...
      const user = await this.database.user.update({
        where: {
          email: dtoCP.email, // username is unique, so we can!
        },
        data: {
          password: hash,
          updatedAt: new Date(),
